...
def build_sam_encoder_vit_b(image_size, num_classes, pixel_mean=[123.675, 116.28, 103.53], pixel_std=[58.395, 57.12, 57.375],
                    checkpoint=None):
    return _build_sam_encoder(
        encoder_embed_dim=768, #768该1536
        encoder_depth=12,
...
def _build_sam_encoder(
    ...
    image_size = image_size
    vit_patch_size = 16 # 32修改16
    image_embedding_size = image_size // 16 # Divide by 16 here #32
    ...
    ) #only need feature extraction encoder
    # sam.eval()
    image_encoder.train()
...
def load_from(sam, state_dict, image_size, vit_patch_size):
    ...
    token_size = int(image_size // vit_patch_size) #32
